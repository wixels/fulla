...
const PropertiesPage: React.FC<Props> = async ({ params: { slug } }) => {
  const properties = await serverClient.org.properties({ slug })
  return (
        ...
        <div className="flex items-center gap-2">
          {properties?.length ? (
            <Link
              ...
              className={buttonVariants({ size: "xs", className: "w-fit" })}
            >
