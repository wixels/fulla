  ...
  async function onSubmit() {
    if (areAllCountsGreaterThanZero(state)) {
      if (
        ...
        listing.roomCount === state.roomCount &&
        listing.bedCount === state.bedCount &&
        listing.bathroomCount === state.bathroomCount
      ) {
        router.push(`/listings/create/${listing.id}/offerings`)
      } else {
  ...
  return (
    <div className="flex flex-col">
      ...
      <ListingFooter progress={22}>
        <Link
          href={`/listings/create/${listing.id}/address`}
